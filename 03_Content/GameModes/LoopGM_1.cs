...
public class LoopGM_1 : GameMode {
    ...
    IEnumerator ReturnCollider(bool isLeft, Collider col)
    {
        ...
        if (isLeft)
        {
            PlayerMgr.Inst.Get("[LEFT]Hand_Grip").Get<AutoDetect>().SubscribeEnterEvent("Wall", () =>
            {
                ...
                PlayerMgr.Inst.Get("[LEFT]Hand_Grip").Get<Rigidbody>().detectCollisions = false;
            });
            ...
            PlayerMgr.Inst.Get("[LEFT]Hand_Grip").Get<Rigidbody>().detectCollisions = true;
        }
        else
        {
            PlayerMgr.Inst.Get("[RIGHT]Hand_Grip").Get<AutoDetect>().SubscribeEnterEvent("Wall", () =>
            {
    ...
    void InitTabBox()
    {
        ...
        obj.transform.localScale = new Vector3(0.06f, 0.06f, 5f);
