          ...
          if (contactBoneCollider is SphereCollider) {
            ...
            int numCollisions = Physics.OverlapCapsuleNonAlloc(point0, point1,
                        ...
                        for (int i = 0; i < numCollisions; i++)
                        {
                            //NotifySoftContactOverlap(contactBone, _softContactColliderBuffer[i]);
                            // Skip soft contact if the object is ignoring contact
                            if (_softContactColliderBuffer[i].attachedRigidbody == null)
                            {
                                continue;
                            }

                            // 잡는 건 아님 .. 미는 것도 아님 .. 콜리더가 안에 있으면 뜬다 .. 
                            // Debug.Log(i.ToString() + " // " + _softContactColliderBuffer[i].name);

                            if (manager.interactionObjectBodies.ContainsKey(_softContactColliderBuffer[i].attachedRigidbody))
                            {
                                if (manager.interactionObjectBodies[_softContactColliderBuffer[i].attachedRigidbody].ignoreContact) continue;
                            }

                            PhysicsUtility.generateCapsuleContact(boneCapsule, 0, _softContactColliderBuffer[i], ref manager._softContacts, ref manager._softContactOriginalVelocities);
                        }
          }
          else {
