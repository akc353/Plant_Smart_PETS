...
public static class GameObjectExtensions
{
    ...
    public static T Get<T>(this GameObject root)
    {
    ...
    public static List<GameObject> GetChilds(this GameObject ob)
    {
        List<GameObject> childs = new List<GameObject>();

        for (int i = 0; i < ob.transform.childCount; i++)
        {
            childs.Add(ob.transform.GetChild(i).gameObject);
        }

        return childs;
    }

    public static Dictionary<string, GameObject> GetAllChilds(this GameObject ob)
    {
    ...
    public static void addDic(GameObject ob, Dictionary<string, GameObject> dic)
    {
        dic.Add(ob.name, ob);

        for (int i = 0; i < ob.transform.childCount; i++)
        {
            addDic(ob.transform.GetChild(i).gameObject, dic);
        }
    }
    public static void AddList<T>(GameObject ob, List<T> list)
    {
        if (!ob.GetComponent<T>().Equals(null))
            list.Add(ob.GetComponent<T>());
        for (int i = 0; i < ob.transform.childCount; i++)
        {
            AddList(ob.transform.GetChild(i).gameObject, list);
        }
    }

    public static List<T> GetComponentInAllChilds<T>(this GameObject ob)
    {
